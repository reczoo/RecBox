...
def compute_match_metrics(user_embs, 
                          item_embs, 
                          train_user2items, 
                          valid_user2items, 
                          query_indices,
...
class Recall(object):
    ...
    def __call__(self, sorted_items, true_items):
        topk_items = sorted_items[:self.topk]
        hit_items = set(true_items) & set(topk_items)
        recall = len(hit_items) / (len(true_items) + 1e-9)
        return recall
...
class nRecall(object):
    ...
    def __call__(self, sorted_items, true_items):
        topk_items = sorted_items[:self.topk]
        hit_items = set(true_items) & set(topk_items)
        recall = len(hit_items) / min(self.topk, len(true_items) + 1e-9)
        return recall
...
class Precision(object):
    ...
    def __call__(self, sorted_items, true_items):
        topk_items = sorted_items[:self.topk]
        hit_items = set(true_items) & set(topk_items)
        precision = len(hit_items) / (self.topk + 1e-9)
        return precision
...
class F1(object):
    ...
    def __call__(self, sorted_items, true_items):
        p = self.precision_k(sorted_items, true_items)
        r = self.recall_k(sorted_items, true_items)
        f1 = 2 * p * r / (p + r + 1e-9)
        return f1
