    ...
    def get_schema(self):
        ...
        for feat, feat_spec in self.feature_map.features.items():
            if feat_spec["type"] == "numeric":
                schema[feat] = tf.io.FixedLenFeature(dtype=tf.float32, shape=1)
            elif feat_spec["type"] in ["categorical", "meta"]:
                schema[feat] = tf.io.FixedLenFeature(dtype=tf.int64, shape=1)
            elif feat_spec["type"] == "sequence":
                schema[feat] = tf.io.FixedLenFeature(dtype=tf.int64, shape=feat_spec["max_len"])
        for label in self.feature_map.labels:
            schema[label] = tf.io.FixedLenFeature(dtype=tf.float32, shape=1)
        return schema

    def input_fn(self, filenames, batch_size=32, shuffle=True):
        def parse_example(example):
            example_dict = tf.io.parse_single_example(example, features=self.get_schema())
            return example_dict
        dataset = tf.data.TFRecordDataset(filenames).map(parse_example, num_parallel_calls=1)
        dataset = dataset.prefetch(buffer_size=10).batch(batch_size, drop_remainder=self.drop_remainder)
        if shuffle:
